(module gen-0-rewards-orchestrator GOVERNANCE
    ...
    (use n_7d47538766e6f80008f253dccd30451f4d483c38.sgk-gen-0-policy [get-nfts-details is-locked update-lock-time])
    (use n_f1c962776331c4773136dc1587a8355c9957eae1.isoko-orchestrator [bulk-claim-weapons-staking])
    ...
    (defconst GEN_0 "battle-heroes")
    (defconst GEN_0_WAIT_PERIOD 604800.0)
    ...
    (defun claim-rewards (account:string tokens:[string])
        ...
        (with-capability (ACCOUNT_GUARD account)
            ...
            (bulk-claim-weapons-staking account (length tokens) (at "guard" (coin.details account)))
        ))
    )
        ...
        (with-capability (ACCOUNT_GUARD account)
            (let*
                (
                    ...
                    (rewardable (get-stake-period (at 'lock-change-time token) GEN_0_WAIT_PERIOD))
                )

(module battle-heroes-nft-policy GOVERNANCE
    ...
    ; Constants
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    (defconst PUBLIC_MINT_TIME "2023-11-06T00:00:00Z")
    (defconst FREE_MINT_TIME "2023-10-29T23:00:00Z")
    ...
    (defconst FREE-MINT-CAP 1076
        @doc "Max unique supply") 
    (defconst MAX-UNIQUE-SUPPLY 5050
        @doc "Max unique supply")
      ...
      ( token:object{token-info}
        ...
        account-info:object{account-wl-record-schema})
            ...
            (let*
                ...
                ;;(current-phase PUBLIC_PHASE)
                ;;(isFreeMintPhase:bool (if (= current-phase FREE_MINT_PHASE) true false))
                ;;(isPublicPhase:bool (if (= current-phase PUBLIC_PHASE) true false))
    
                ;;(free-mints-remaining:integer (at 'free-mints-remaining account-info))
                ;;(hasFreeMintsLeft:bool (if (> free-mints-remaining 0) true false))
                ;;(using-free-mint:bool ( and (= true hasFreeMintsLeft) (= true isFreeMintPhase)))
    
                ;;(can-mint:bool (or (= true isPublicPhase) ( and (= true hasFreeMintsLeft) (= true isFreeMintPhase) ) ))
                (price:decimal (get-price-by-phase PUBLIC_PHASE false))
                )
                ;(enforce (= true can-mint) "Mint is not live right now, or max phase cap reached.")
    ...
    (defun get-price-by-phase:decimal(current-phase:string hasFreeMintsLeft:bool)
        (cond
            ( (and (= FREE_MINT_PHASE current-phase) (= hasFreeMintsLeft true)) 0.0)
            ((= PUBLIC_PHASE current-phase) 19.4)
            10000000.01)
    )
    ...
    (defun get-current-phase:string()
        ...
        (let*
            (
                ...
                (isFreeMintStarted (check-is-mint-live-for-phase FREE_MINT_TIME))
                (post-increment-mint-supply:integer (get-minted-supply))
            )
            (cond
                ((= true isPublicStarted) PUBLIC_PHASE)
                ( (and (= true isFreeMintStarted) (<= post-increment-mint-supply FREE-MINT-CAP)) FREE_MINT_PHASE)
                "Mint is not Live now, or Max phase cap reached!")
        )
    )
